...
import {
  AgentStudentAPIResponse,
  getAgentStudents,
  searchAgentStudents,
} from "@/api/agentStudents";
...
const AgentStudents = () => {
  ...
  const getStudentQuery = useQuery<AgentStudentAPIResponse>({
    queryKey: ["students", pageCount, filter],
    queryFn: () => getAgentStudents(pageCount, filter),
  });

  const searchStudentQuery = useQuery<AgentStudentAPIResponse>({
    queryKey: ["searchStudents", searchParam],
    queryFn: () => searchAgentStudents(searchParam!),
  });
  ...
  return (
    <>
      ...
      <div
        className={`w-full bg-blue-50 h-max p-4 rounded mb-1.5 transition-all duration-200 ${
          showFilter ? "hidden" : "flex"
        } flex flex-col justify-between sm:flex-col md:flex-row lg:flex-row gap-2`}
      >
      ...
      {getStudentQuery.data?.students.length === 0 &&
        !getStudentQuery.isError && <NoData text="No Students" />}
                  ...
                  <TableRow key={item.id || "-"}>
                    <TableCell
                      ...
                      onClick={(
                        ...
                        nav(`/agentStudents/detail/${item.id}`);
                      }}
                    >
                    ...
                    <TableCell className="text-center">
                      <button
                        onClick={(
                          ...
                          nav(`/agentStudents/print/${item.id}`);
                        }}
                        className="btn btn-anim bg-primaryColor text-sm text-white w-full"
                      >
                        Print
                      </button>
                    </TableCell>
                    ...
                    {user?.user.roles.includes("Admin") ||
                    user?.user.roles.includes("Agent") ? (
                      <TableCell className="text-center flex flex-row justify-center items-center gap-x-1">
                        {!item.isActive && (
                          <button
                            onClick={(
                              event: React.MouseEvent<HTMLButtonElement>
                              ...
                              event.preventDefault();
                              nav(`/agentStudents/activate/${item.id}`);
                            }}
                            className="btn-anim bg-primaryColor text-white p-2 rounded"
                          >
                            Activate
                          </button>
                        )}
                        <button
                          onClick={(
                            ...
                            nav(`/agentStudents/edit/${item.id}`);
                          }}
                          className="btn-anim"
                        >
                        ...
                        <button
                          onClick={(
                            ...
                            nav(`/agentStudents/editAddress/${item.id}`);
                          }}
                          className="btn-anim"
                        >
